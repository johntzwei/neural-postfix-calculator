...
def preliminary_seq2seq(input_shape, output_shape):
    names = [ 'simple-seq2seq-10' ]
    nnets = [ seq2seq(input_shape, output_shape, hidden_dims=[10]) ]
    cmp_all(nnets)
...
def seq2seq(input_shape, output_shape, hidden_dims=[10], dropout=0.25):
    model = SimpleSeq2Seq(input_dim=input_shape[1], hidden_dim=hidden_dims[0], output_length=output_shape[0], output_dim=output_shape[1])
    return model
